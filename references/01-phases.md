...
- new maintainer, reviewer, bot, or ClawSweeper comments
...
OpenClaw runtime, Node, package-manager, build, and test proof should use the operator-approved OpenClaw test environment for that run. Do not spend time probing unrelated local machine tooling unless local proof is explicitly allowed. If no suitable test-environment GO exists, stop and ask for the exact PROOF-GO.
...
If ClawSweeper, a maintainer, or CI requests changes:
...
Forbidden:
- GitHub unless the operator allows it.
...
Purpose:
- Find current, editable OpenClaw bug or stability PR candidates.
...
- maintainer and ClawSweeper direction
...
Purpose:
- Review external or official OpenClaw PRs read-only for relevance and risk to integrations or agent setups.

Allowed:
- PR metadata read-only
- PR diff read-only
- related issue/review/CI summary read-only, if allowed
- risk estimate for gateway, auth, sessions, channels, plugins, config, runtime
