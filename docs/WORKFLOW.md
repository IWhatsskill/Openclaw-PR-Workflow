...
- `Real environment tested` must name the actual affected runtime or layer, such as local OpenClaw runtime, plugin handler integration, CLI command, browser UI, test server, live transport, Gateway/API path, or queue/runtime path.
...
- For fork PRs, prefer `git push fork HEAD:<branch>`.
