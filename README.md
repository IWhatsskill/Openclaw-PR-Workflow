...
This skill helps an agent do PR work without jumping ahead. It separates discovery, ownership checks, inspection, patching, proof, PR body work, and publishing into explicit phases. Each phase needs its own operator GO.

## At A Glance

- Phase-gated PR workflow for OpenClaw work.
- Read-only discovery before any patching.
- Explicit operator approval for each write action.
- Built-in templates for PR bodies, proof notes, and handoff reports.
- Designed to keep agents conservative around ownership, duplicates, tests, and publishing.

## Who This Is For

Use this skill when an agent should help with OpenClaw PR work while staying conservative around duplicates, ownership, proof quality, and write actions.

Good use cases:

- Find current OpenClaw bug or stability candidates.
- Check whether an issue is free to work on before reading code.
- Inspect a candidate and define the smallest safe fix.
- Prepare a patch only after the issue is free and the checkout is ready.
- Prove the change before preparing a PR body.
- Publish only one explicitly approved write action.
- Review official or external OpenClaw PRs in read-only mode.
...
- GitHub or tracker checks unless allowed
- code reads unless the operator allowed that scope
- tests, patches, proof, or writes

### Phase 2: SCOUT

Purpose: find current, editable OpenClaw bug or stability candidates.

Use when:

- the operator wants fresh PR candidates
- the agent should search issues and PR metadata read-only
- the goal is candidate discovery, not fixing

Allowed:

- read-only issue and PR metadata
- dashboard, label, and search-window checks
...
- the operator names one issue for ownership and duplicate checks

Allowed:

- read issue metadata and comments
- search open and closed duplicate/canonical PRs
- check assignee, claim, maintainer direction, and stop labels
...
- write the body to GitHub or a tracker
...
- risk estimate for gateway, auth, sessions, channels, plugins, config, migrations, defaults, runtime, and fallback behavior
...
- hard stop label or maintainer direction
